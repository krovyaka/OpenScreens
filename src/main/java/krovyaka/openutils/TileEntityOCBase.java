...
public class TileEntityOCBase extends TileEntity implements ITickable, ManagedEnvironment {
    ...
    public TileEntityOCBase(String name) {
        super();
        componentName = name;
    }

    public TileEntityOCBase(String name, EnvironmentHost host) {
        isUpgrade = true;
        componentName = name;
        container = host;
        setupNode();
    ...
    protected void initOCFilesystem(String path, String name) {
        oc_fs = li.cil.oc.api.FileSystem.asManagedEnvironment(li.cil.oc.api.FileSystem.fromClass(OpenUtils.class, OpenUtils.MODID, path), name);
